...
// --------------------------------------------------------------------------
//
// 17/10/2002 - EJB di connessione alla procedura SINS Tabella clonabili
//
// sara cossu
//
// ==========================================================================
...
public class RuoliOperatoreEJB extends SINSSNTConnectionEJB  {



public RuoliOperatoreEJB() {}
...
public ISASRecord queryKey(myLogin mylogin,Hashtable h) throws  SQLException {
    ...
    try{
		    ...
		    String myselect="Select * from clonabili where"+
 				" kuser='"+(String)h.get("kuser")+"'";
...
public Vector query(myLogin mylogin,Hashtable h)
throws  SQLException {
   /* boolean done=false;
    ISASConnection dbc=null;
    try{
        dbc=super.logIn(mylogin);
        String myselect="Select * from clonabili where descr"+
	" like '"+(String)h.get("descr")+"%' ORDER BY descr";
...
*/
    ...
    try{
        dbc=super.logIn(mylogin);
        String myselect="Select * from clonabili";

       //controllo valore corretto descr

        scr=(String)(h.get("descr"));
	    if (!(scr==null))
            if (!(scr.equals(" ")))
              {
               scr=duplicateChar(scr,"'");
               myselect=myselect+" where descr like '"+scr+"%'";
              }
        myselect=myselect+" ORDER BY descr";
        ISASCursor dbcur=dbc.startCursor(myselect);
	    Vector vdbr=dbcur.getAllRecord();
	    dbcur.close();
		dbc.close();
...
public ISASRecord insert(myLogin mylogin,Hashtable h) throws DBRecordChangedException, ISASPermissionDeniedException, SQLException {
    ...
    try{
        dbc=super.logIn(mylogin);
        ISASRecord dbr=dbc.newRecord("clonabili");
