...
// ----------------------------------------------------------------------------
//
// 07/12/2006 - EJB di connessione alla procedura SINS Tabella FoInfEleInvEJB
//
// Jessica Caccavale
//
//gb 01/08/07: Se si sono salvate dei record senza
//	       l'inviante (ski_inviato), allora viene fatta stampare la dicitura di
//	       'NON SPECIFICATO'. E' stato necessario fare cos� perch� se dalla query
//	       risultavano tutti record senza inviante specificato, allora dava errore
//	       di 'rendering' in stampa. (vedi anche FoInfEleMotEJB).
// ============================================================================
...
public class FoInfEleInvEJB extends SINSSNTConnectionEJB {

public FoInfEleInvEJB() {}
...
throws SQLException {
	...
	String type = par.get("TYPE")!=null?par.get("TYPE").toString():"PDF";
		...
		else stampaExcel(dbc,dbcur,par,doc);
                doc.write("finale");
                doc.close();
                dbcur.close();
                dbc.close();
...
private void stampaExcel(ISASConnection dbc, ISASCursor dbcur, Hashtable par, mergeDocument doc)
    throws Exception{
			...
			System.out.println("FoInfEleInv: 2");
		...
		p.put("#data_ini#", "("+getDateField(dbr, "ski_data_apertura")+")");
		p.put("#data_fine#", "("+getDateField(dbr, "ski_data_uscita")+")");
		p.put("#des_inviato#",getMotivoUscita(dbc,dbr.get("ski_inviato").toString()));
		if (dbr.get("n_cartella")==null)
                    p.put("#n_cartella#", " ");
		else
              p.put("#n_cartella#",((Integer)dbr.get("n_cartella")).toString());
              			System.out.println("FoInfEleInv: 8");
        ...
        while (enumDistretti.hasMoreElements())
        {
              ...
              while (enumComuni.hasMoreElements())
              {
                 ...
                 while (enumTipologia.hasMoreElements())
                 {
                   ...
                   System.out.println("INVIANTE:"+motivo);
...
//                          pca="Presidio:";
                  ...
                  if (motivo.equals("NESSUNA DIVISIONE")){
                        htab.put("#inviato#","");
                        htab.put("#des_inviato#","");
                  }
                  else{ htab.put("#inviato#","Soggetto inviante");
                        htab.put("#des_inviato#",getMotivoUscita(dbc,motivo));
                  }
...
private String mkSelectAnalitica(ISASConnection dbc, Hashtable par) {
        ...
        if(sUte.equals("1"))
           tipologia= " s.ski_inviato,";
        else  tipologia= " 'NESSUNA DIVISIONE' ski_inviato ,";
	...
	String motivo="";
        if (par.get("inviato") != null && !((String)par.get("inviato")).equals("0")){
          motivo=(String)(par.get("inviato"));
          if (!motivo.equals(""))
                myselect=myselect+" AND s.ski_inviato='"+motivo+"'";
        }
        myselect+=" ORDER BY s.ski_inviato,c.cognome,c.n_cartella";
	return myselect;
}
...
throws SQLException{
        ...
        try {
                 ...
                 String sel="SELECT descrizione tab_descrizione FROM segnala "+
                            " WHERE codice='"+codice +"'";
...
throws SQLException
{
	String  motivo=""; //gb 01/08/07
        try
        {
              Vector vDati=new Vector();
            if (dbr.get("ski_inviato")!=null && !((String)dbr.get("ski_inviato")).equals(""))
            {
                motivo=(String)dbr.get("ski_inviato");
/*gb 01/08/07 *******
...
*gb 01/08/07: fine *******/
	      ...
	      {
		motivo = "00";  //gb 01/08/07: Codice inviante non esistente nella tabella di decodifica
...
//gb 01/08/07: fine *******
		...
		e.printStackTrace();
		throw new SQLException("Errore eseguendo una Analizzainviato");
	}
}
...
private void preparaLayout(mergeDocument md, ISASConnection dbc,String data_inizio,String data_fine,Hashtable par) {
	...
	try {
        ...
        ServerUtility su =new ServerUtility();
        System.out.println("****PreparaLayout");
