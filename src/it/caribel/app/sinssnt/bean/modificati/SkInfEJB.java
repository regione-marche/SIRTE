...
// --------------------------------------------------------------------------
//
// 18/09/2002 --------EJB Scheda Infermieri----
// Barbara Giannattasio/Jessica Caccavale
...
public class SkInfEJB extends SINSSNTConnectionEJB  
{
	...
	public ISASRecord queryKey(myLogin mylogin,Hashtable h) throws SQLException, CariException {
		...
		try 
		{
			...
			data_apertura = ISASUtil.getValoreStringa(h,"ski_data_apertura");    
		}
		catch (Exception e)
		{
			e.printStackTrace();
			throw new SQLException("SkInf queryKey: Errore: manca la chiave primaria");
		}
		...
		try
		{
			...
			String strDtSkVal = (String)h.get("pr_data"); // 17/04/08
			// 20/12/11
			if (strDtSkVal == null && ManagerDate.validaData(data_apertura)) {
				ISASRecord recProg = getProgetto(dbc, n_cartella, data_apertura);
				if ((recProg != null) && (recProg.get("pr_data") != null))
					strDtSkVal = ((java.sql.Date)recProg.get("pr_data")).toString();
			}

			
			if(h.get("dimiss")!=null) {
				dimiss=(String)h.get("dimiss");
			}
			
			String myselect=" Select s.* from skinf s where s.n_cartella= "+n_cartella;
			if (ISASUtil.valida(n_contatto)){
				myselect +=" and s.n_contatto="+n_contatto;
			}
			if (ManagerDate.validaData(dtAssistitoChiusura)){
				myselect +=" and s.n_contatto in (select max(n_contatto) from skinf x where x.n_cartella = s.n_cartella) ";
			}
...
//				//bargi 05/12/2006
//				dbr.put("ski_data_adl",CaricaData(dbc, n_cartella, dbr.get("ski_data_apertura"), "sc_adl", dbr.get("ski_data_uscita")));
//				dbr.put("ski_data_pfeiffer",CaricaData(dbc, n_cartella, dbr.get("ski_data_apertura"), "sc_pfeiffer", dbr.get("ski_data_uscita")));
//				dbr.put("ski_data_iadl",CaricaData(dbc, n_cartella, dbr.get("ski_data_apertura"), "sc_iadl",dbr.get("ski_data_uscita")));
//				dbr.put("ski_data_caregiver",CaricaData(dbc, n_cartella, dbr.get("ski_data_apertura"), "sc_caregiver", dbr.get("ski_data_uscita"), "skcg_data"));
//				dbr.put("ski_data_braden",CaricaData(dbc, n_cartella, dbr.get("ski_data_apertura"), "sc_braden", dbr.get("ski_data_uscita"), "skb_data"));
//				dbr.put("ski_data_tiq",CaricaData(dbc, n_cartella, dbr.get("ski_data_apertura"), "sc_tiq", dbr.get("ski_data_uscita")));
//				// fine 05/12/06
//				dbr.put("ski_data_barthel",CaricaData(dbc, n_cartella, dbr.get("ski_data_apertura"), "sc_barthel", dbr.get("ski_data_uscita"),"skbt_data"));

				Hashtable h1 = dbr.getHashtable();
				LOG.debug("Hashtable=>"+h1.toString());
				
				if (h1.get("ski_operatore")!=null && !((String)h1.get("ski_operatore")).equals(""))
				{
					dbr.put("desc_operat", decodifica("operatori","codice",dbr.get("ski_operatore"),
							"nvl(trim(cognome),'') ||' '  ||nvl(trim(nome),'')",dbc));
				}
				else dbr.put("desc_operat","");

				if (h1.get("ski_infermiere")!=null && !((String)h1.get("ski_infermiere")).equals(""))
				{
					dbr.put("desc_inf", decodifica("operatori","codice",dbr.get("ski_infermiere"),
							"nvl(trim(cognome),'') ||' '  ||nvl(trim(nome),'')",dbc));
				}
				else dbr.put("desc_inf","");

				if (h1.get("ski_inviato")!=null && !((String)h1.get("ski_inviato")).equals(""))
				{
					dbr.put("desc_inviato", decodifica("segnala","codice",
							dbr.get("ski_inviato"),"descrizione",dbc));
				}
				else dbr.put("desc_inviato", "");

				if (h1.get("ski_tipout")!=null && !((String)h1.get("ski_tipout")).equals(""))
				{
					dbr.put("desc_tipout", decodifica("tipute_s","codice",
							dbr.get("ski_tipout"),"descrizione",dbc));
				}
				else dbr.put("desc_tipout", "");

				if (h1.get("ski_osp_dim")!=null && !((String)h1.get("ski_osp_dim")).equals(""))
				{
					dbr.put("desc_ospdim", decodifica("ospedali","codosp",
							dbr.get("ski_osp_dim"),"descosp",dbc));
				}
				else dbr.put("desc_ospdim", "");

				if (h1.get("ski_uo_dim")!=null && !((String)h1.get("ski_uo_dim")).equals(""))
				{
					dbr.put("desc_rep", decodifica("reparti","cd_rep",
							dbr.get("ski_uo_dim"),"reparto",dbc));
				}
				else dbr.put("desc_rep", "");

				/*gb 02/05/07 *******
	            String selg="Select * from skieventi where "+
				...
				//06/04/2007 bargi ,skpa_data_chiusura 
	            String myprest="SELECT skpa_data,skpa_progetto,skpa_data_chiusura FROM skiprogass WHERE"+
				...
				dbr.put("dimiss", dimiss);
				
				// 05/02/13 
				dbr.put("desc_presidio", decodPresidio(dbc, (String)dbr.get("ski_cod_presidio"), (String)dbr.get("ski_infermiere")));
		...
		{
			e.printStackTrace();
			throw new SQLException("Errore eseguendo una queryKey()  ",e);
		}
		finally
		{
			if(!done)
	...
	public ISASRecord getContattoInfCorrente(myLogin mylogin, Hashtable h)throws SQLException, CariException
	{
		LOG.info("SkInfEJB: getContattoInfCorrente " + h.toString());
		...
		try
		{
			...
			if (strDtSkVal == null)
			{
				LOG.debug("SkInfEJB -->> getContattoInfCorrente: dataSkVal NULLA!!");
				done = true;
				return dbr;
			}
			//gb 27/08/07 ---
			...
			LOG.info(" abilitazione dal CONF: " + abilMultiCont);

			// 15/07/08 -----
			String zonaOper = getZonaOper(dbc, (String)h.get("ski_operatore"));
			...
			String myselect = "SELECT s.* FROM skinf s" +
			...
			if (multiCont)
				myselect += " AND s.ski_data_apertura IN (SELECT MAX(a.ski_data_apertura) FROM skinf a" +
				" WHERE a.n_cartella = s.n_cartella" +
				" AND a.ski_data_uscita IS NULL)";

			LOG.debug("SkInfEJB/getContattoInfCorrente : " + myselect);
			...
			{
				Hashtable h1 = dbr.getHashtable();
				LOG.info("Hashtable=>"+h1.toString());
				if (h1.get("ski_operatore")!=null && !((String)h1.get("ski_operatore")).equals(""))
				{
					dbr.put("desc_operat", decodifica("operatori","codice",dbr.get("ski_operatore"),
							"nvl(trim(cognome),'') ||' '  ||nvl(trim(nome),'')",dbc));
				...
				if(gestore_segnalazioni.isSegnalDaGestire(dbc, GestCasi.CASO_SAN))
				{
					...
					int caso = prendi_dati_caso(dbc,dbr);
					LOG.debug("CASO DEL CONTATTO CORRENTE: " + caso);
					if(prendi_segnalazione(dbc,caso,dbr))
						prendi_presacarico(dbc,caso,dbr);
				}
				
				LOG.debug("SkInfEJB.getContattoInfCorrente DBR finale: " + dbr.getHashtable().toString());
			}
		...
		{
			LOG.error("SkInfEJB.getContattoInfCorrente(): "+e);
			throw new CariException(CostantiSinssntW.MSG_ECCEZIONE_DIRITTI_MANCANTI, -2);
		}
		catch(Exception e)
		{
			e.printStackTrace();
			throw new SQLException("Errore eseguendo SkInfEJB/getContattoInfCorrente()  ",e);
		}
		finally
		{
			if(!done)
	...
	*/
	...
	{
		LOG.info(" gestione_segnalazione -- HASH: " +  h.toString() + " REC: " + dbr.getHashtable().toString());
		
		int stato_caso = -1;
		int id_caso = -1;
		
		h.put("operZonaConf", (String)dbr.get("ski_operatore")); // 15/10/09
		...
		if(prov.equals("insert") && (id_caso == -1 || stato_caso == GestCasi.STATO_CONCLU))
		{
			...
			try 
			{
				...
				if(h.get("dt_segnalazione") == null || h.get("dt_segnalazione").equals(""))
					h.put("dt_segnalazione", h.get("ski_data_apertura"));
			...
			{	
				LOG.error("SkInfEJB gestione_segnalazione, insert -- " + e);
				throw e;
			} 
			catch (DBRecordChangedException e) 
			{	
				LOG.error("SkInfEJB gestione_segnalazione, ERRORE REPERIMENTO CHIAVE! -- " + e);
				return id_caso;
			} 
			catch (ISASPermissionDeniedException e) 
			{	
...
// 29/03/10	else if(id_caso != -1 && (stato_caso != GestCasi.STATO_CONCLU &&  stato_caso != -1))
		else if(id_caso != -1 && (stato_caso != -1))
		{
			...
			try 
			{
				...
				LOG.debug(" gestione_segnalazione - UPDATE, H: " + h.toString());
				
				// 12/08/10 m ---------------
				if(dbr.get("origine") != null && !dbr.get("origine").equals("")) {
					int origine = Integer.parseInt(dbr.get("origine").toString());
					LOG.debug("gestione_segnalazione: Origine del caso "+id_caso+" =["+origine+"]");
					...
					if(origine == GestCasi.CASO_SAN) { // 12/08/10 m ---		
						...
						if (!esistePresaCar(dbc, h)) {
							if(h.get("dt_presa_carico") == null || h.get("dt_presa_carico").equals(""))
								h.put("dt_presa_carico", h.get("ski_data_apertura"));
			...
			{	
				LOG.error("SkInfEJB gestione_segnalazione, update -- " + e);
				throw e;
			} 
			catch (Exception e) 
			{
	...
	private void prendi_presacarico(ISASConnection dbc, int caso,ISASRecord dbr)
	{
		...
		{
			LOG.error("SkInfEJB prendi_presacarico, ERRORE REPERIMENTO CHIAVE! -- " + e1);
		}
		catch (Exception e) 
		{
			LOG.error("SkInfEJB prendi_presacarico, fallimento! -- " + e);
		}
	}
	...
	private boolean prendi_segnalazione(ISASConnection dbc, int caso,ISASRecord dbr)
	{
		...
		{
			LOG.error("SkInfEJB prendi_segnalazione, ERRORE REPERIMENTO CHIAVE! -- " + e1);
			return false;
		}
		catch (Exception e) 
		{
			LOG.error("SkInfEJB prendi_segnalazione, fallimento! -- " + e);
			return false;
		}
	}
	...
	private int prendi_dati_caso(ISASConnection dbc, ISASRecord dbr)
	{
		...
		try 
		{
			...
			LOG.info("SkInfEJB -- prendi dati caso: " + h.toString());
		...
		{
			LOG.error("SkInfEJB prendi_dati_caso, manca chiave primaria! -- " + e);
			return -1;
		}
		catch (Exception e) 
		{
			LOG.error("SkInfEJB prendi_dati_caso, fallimento! -- " + e);
			return -1;
		}
	}
	...
	private int prendi_dati_casoOrigine(ISASConnection dbc, ISASRecord dbr)
	{
		...
		try 
		{
			...
			LOG.info("SkInfEJB -- prendi dati caso x origine: " + h.toString());
	...
	public Vector query(myLogin mylogin,Hashtable h) throws  SQLException {
		...
		try{
			dbc=super.logIn(mylogin);
			String myselect="SELECT * FROM skinf WHERE n_cartella="+
			(String)h.get("n_cartella")+
			" ORDER BY n_contatto,ski_data_apertura DESC ";
	...
	public Vector query_loadGridSkInf(myLogin mylogin,Hashtable h) throws  SQLException {
		...
		try{
			// 26/10/06 ---
			if (strDtSkVal == null){
				LOG.debug("\nSkInfEJB -->> query_loadGridSkInf: dataSkVal NULLA!!");
				done = true;
				return vdbr;
			}
			// 26/10/06 ---
			...
			String critDtChius = " AND s.ski_data_uscita IS" + (contAperti?"":" NOT") + " NULL";
			if (contAperti)
				critDtChius += " AND s.ski_data_apertura NOT IN (SELECT MAX(a.ski_data_apertura) FROM skinf a" +
				" WHERE a.n_cartella = s.n_cartella" +
				" AND a.ski_data_uscita IS NULL)";
			// 14/04/08 -------------------------
...
//			" AND pc.prc_n_contatto = s.n_contatto" +
			...
			e.printStackTrace();
			throw new SQLException("Errore eseguendo la query_loadGridSkInf()  ");
		}
		finally{
			if(!done){
				try{
	...
	private void decodificaQueryInfo(ISASConnection mydbc, Vector vdbr) throws Exception
	{
		for (int i=0; i<vdbr.size(); i++)
		{
			...
			decodificaQueryOperatore(mydbc, dbr, "ski_operatore", "operatore_apertura");
			decodificaQueryOperatore(mydbc, dbr, "ski_infermiere", "operatore_infermiere");
			dbr.put("ski_tipocura_desc", (String)util.getDecode(mydbc, "tab_voci", "tab_cod", "tab_val", "SAOADI", (String)dbr.get("ski_tipocura"), "tab_descrizione"));
		}
	}
	//gb 02/07/07: fine *******
...
/** 26/03/13: spostato in metodo apposito
			...
			" n_cartella="+(String)h.get("n_cartella")+
			" and n_contatto="+(String)h.get("n_contatto")+
			//" and ski_data_apertura="+formatDate(dbc,(String)h.get("ski_data_apertura"))+
...
**/			
	...
	throws SQLException {
		...
		try {
			...
			data_apertura=(String)h.get("ski_data_apertura");
		}catch (Exception e){
			e.printStackTrace();
		...
		try{
			dbc=super.logIn(mylogin);
			String myselect="Select n_cartella,n_contatto,ski_data_apertura from skinf where "+
			" n_cartella="+n_cartella+
			" and n_contatto="+n_contatto+
			" and ski_data_apertura="+formatDate(dbc,data_apertura);
			LOG.info("select query_salvataggio su skinf==="+myselect);
			ISASRecord dbr=dbc.readRecord(myselect);
			dbc.close();
			super.close(dbc);
			done=true;
	...
	public Vector queryPaginate(myLogin mylogin,Hashtable h) throws  SQLException {
		...
		ISASConnection dbc=null;
		try{
			LOG.info("SkInfEJB -- QueryPaginate()"+h.get("tipo"));
			ServerUtility su=new ServerUtility();
			dbc=super.logIn(mylogin);
			
			String myselect="SELECT m.mde_numinv,p.tau_des,m.mde_datcons, m.mde_datscad "+
			...
			LOG.info("queryPaginate SkInf: "+myselect);
			ISASCursor dbcur=dbc.startCursor(myselect);
...
//		String strDataApeContatto = (String) h.get("ski_data_apertura");
		String strNCartella = ISASUtil.getValoreStringa(h,"n_cartella");
		String strDataApeContatto = ISASUtil.getValoreStringa(h,"ski_data_apertura");
// 12/02/13	String strNContatto = (String) h.get("n_contatto"); // 13/09/07 m.
		...
		String mySel = "SELECT ski_data_uscita" +
		" FROM skinf" +
		" WHERE n_cartella = " + strNCartella +
		(ISASUtil.valida(strNContatto) ?" AND n_contatto <> "+strNContatto:"") + // 13/09/07 m.
		" AND ski_data_uscita >= " + formatDate(dbc, strDataApeContatto) +
		" AND ski_data_uscita IS NOT NULL";
...
//		String strDataApeContatto = (String) h.get("ski_data_apertura");
		...
		LOG.info("SkInfEJB / checkDateContEDatePianoAssist / mySel: " + mySel);
		...
		{
			dbcur = null;
			if (h.get("ski_data_uscita") != null)
			{
				strDataChiuContatto = "" + h.get("ski_data_uscita");
	...
	private String scriviProgetto(ISASConnection mydbc, String numCart, String dtSkVal) throws Exception {
		...
		LOG.info("\n SkInfEJB -->> insert: Inserito record su tabella PROGETTO");
		return dtSkVal;
	}
	...
	private void scriviProgettoCont(ISASConnection mydbc, String numCart, String dtSkVal, String tpOper, String numCont) throws Exception
	{
		...
		LOG.info("\n SkInfEJB -->> scriviProgettoCont: Inserito record su tabella PROGETTO_CONT");
	}
	...
	public ISASRecord insert(myLogin mylogin, Hashtable h) throws DBRecordChangedException, ISASPermissionDeniedException, SQLException, CariException {
		LOG.info("SkInfEJB: insert HASH -- " + h.toString());
		boolean done = false;
		ISASConnection dbc = null;
		ISASCursor dbcur = null;// 07/12/06 m.
		...
		catch(CariException ce)
		{
			...
			try
			{
				LOG.error("SkInfEJB.insert() => ROLLBACK");
		...
		finally
		{
			if(!done)
			{
				...
				catch(Exception e2){   LOG.error("SkInfEJB: Errore eseguendo una insert() --> " + e2);   }
			}
		}
	}
	...
	public ISASRecord insertTransactional(ISASConnection dbc, Hashtable h) throws SQLException,
		...
		try {
			...
			data_apertura = h.get("ski_data_apertura").toString();
//			dimissOsp=(String)h.get("dimiss");
		...
		// 10/01/12 m ---
		ISASRecord dbr = insertNew(dbc, h, n_cartella, strDtSkVal);
		...
		String idSkso = ISASUtil.getValoreStringa(h, "id_skso");
		if (ISASUtil.valida(idSkso)){
			Hashtable h_rm_diario = (Hashtable)h.clone();
			h_rm_diario.put("tipo_operatore", CostantiSinssntW.TIPO_OPERATORE_INFERMIERE);
	...
	// 10/01/12 m
	public ISASRecord insertNew(ISASConnection dbc, Hashtable h, String n_cartella, String strDtSkVal) throws Exception
	{
		LOG.info("SkInfEJB: insert HASH -- " + h.toString());

		String dimissOsp = (String)h.get("dimiss");
		String data_apertura=(String)h.get("ski_data_apertura");
		...
		ISASRecord dbr=dbc.newRecord("skinf");
		...
		if ((gestore_casi.isUbicazRegVeneto(dbc, h)).booleanValue()) {	
			if ((h.get("ski_descr_contatto") == null) || ("".equals((String)h.get("ski_descr_contatto"))))
				dbr.put("ski_descr_contatto", "CONTATTO INFERMIERISTICO N."+iProgressivo);
		}
						
		// 09/10/12
		if ((h.get("ski_modalita") == null) || ("".equals((String)h.get("ski_modalita"))))
			dbr.put("ski_modalita", "1");
			
			
		dbc.writeRecord(dbr);

		ISASUtil.checkUnicoContattoAperto(dbc, h, n_contatto, "skinf", "ski_data_uscita", Labels.getLabel("contatti.msg.contattoUnicoApertoViolato"));

		
		// 30/04/07: scrittura su PROGETTO_CONT ed, eventualmente, PROGETTO -----
		if ((strDtSkVal == null) || ((strDtSkVal != null) && (strDtSkVal.trim().equals(""))))
		{
			ISASRecord progetto = getProgetto(dbc, n_cartella, dbr.get("ski_data_apertura").toString());
			if (progetto == null)			
		...
		String myselect="Select * from skinf where n_cartella="+n_cartella+" and " +
...
//			// fine 05/12/06
//			dbr.put("ski_data_barthel",CaricaData(dbc, n_cartella, dbr.get("ski_data_apertura"), "sc_barthel", dbr.get("ski_data_uscita"),"skbt_data"));
			String descr_infer=(String)dbr.get("ski_descr_contatto");
			String data_chiusura="";
			
			if (dbr.get("ski_data_uscita")!=null && !((dbr.get("ski_data_uscita"))).equals(""))
				data_chiusura=((java.sql.Date)dbr.get("ski_data_uscita")).toString();

			String selref="SELECT * FROM skinf_referente WHERE "+
			...
			if(dbref==null)
			{
				if(dbr.get("ski_infermiere")!=null && !((String)dbr.get("ski_infermiere")).equals(""))
				{
					String infref=(String)dbr.get("ski_infermiere");
					String data_ref="";
				
					if (dbr.get("ski_infermiere_da")!=null)
						data_ref=((java.sql.Date)dbr.get("ski_infermiere_da")).toString();
					
					this.insertInfRef(dbc,infref,data_ref,data_apertura,n_cartella,n_contatto);
				}
			}
	
			/*gb 07/05/07 ******* eliminato tabella CONTSAN
		        String operatore=(String)dbr.get("ski_operatore");
		        String selcont="SELECT * FROM contsan where n_cartella="+n_cartella+
		                       " AND n_contatto="+n_contatto;
		        LOG.info("Su contsan"+selcont);
		        ISASRecord dbcont=dbc.readRecord(selcont);
		        if(dbcont!=null)
		          this.updateContsan(dbc,data_apertura,data_chiusura,descr_infer,n_cartella,n_contatto);
		        else
		          this.insertContsan(dbc,data_apertura,data_chiusura,descr_infer,n_cartella,operatore);
			gb 07/05/07 *******/
	
			if((dimissOsp != null) && (!dimissOsp.trim().equals("N")))// 04/06/08
				AggiornaDimissione(dbc, n_cartella, dimissOsp, dbr);
			...
			// 05/02/13 
			dbr.put("desc_presidio", decodPresidio(dbc, (String)dbr.get("ski_cod_presidio"), (String)dbr.get("ski_infermiere")));			
			if (dbr.get("ski_operatore")!=null && !((String)dbr.get("ski_operatore")).equals(""))
			{
				dbr.put("desc_operat", decodifica("operatori","codice",dbr.get("ski_operatore").toString(),
						"nvl(trim(cognome),'') ||' '  ||nvl(trim(nome),'')",dbc));
			}
			
			LOG.info("SkInfEJB: insert -- DBR restituito === " + dbr.getHashtable().toString());
		}
	...
	public void gestSegnalNew(ISASConnection dbc, ISASRecord dbr, Hashtable h) throws Exception
	{
		...
		int idCaso = -1;
		if ((dbr.get("ski_modalita") != null) && ("1".equals((String)dbr.get("ski_modalita")))) {
			if (!"ELECASI".equals(proven)) { // 14/05/10: se non provengo da "ElencoCasi"
				...
				if (gestore_segnalazioni.isSegnalDaGestire(dbc, GestCasi.CASO_SAN))
				{
					...
					LOG.info("gestSegnalNew() -- Fatta gestione segnalazione per il caso: " + idCaso);						
				} else { 
/*** 26/06/13 m: NON SERVE PIU	***			
					// 08/10/10 m: solo x PIEMONTE
					if(h.get("ski_dtsegnalazione") == null || h.get("ski_dtsegnalazione").equals(""))
						h.put("dt_segnalazione", h.get("ski_data_apertura"));
					else
						h.put("dt_segnalazione", h.get("ski_dtsegnalazione"));
				
					if(h.get("ski_dtpresacarico") == null || h.get("ski_dtpresacarico").equals(""))
						h.put("dt_presa_carico", h.get("dt_segnalazione"));
					else
						h.put("dt_presa_carico", h.get("ski_dtpresacarico"));
					...
					h.put("operZonaConf", (String)h.get("ski_operatore"));
...
*** 26/06/13 m: NON SERVE PIU ***/
			...
			{
				LOG.info("insert() -- Controllo contatto UNICO SANITARIO H == " + h.toString());
							
				boolean unico = gestore_casi.query_checkUnicoContAperto(dbc, h, true, true);
				if (idCaso != -1 && unico)
				{
					LOG.info("insert() -- Gestisco la chiusura del caso" );
					...
					if (origine != -1)
					{
						LOG.info("insert() -- Origine del caso: " + origine + " - stato = " + statoCaso);
							...
							|| (((Boolean)gestore_casi.isUbicazRegTosc(dbc, h)).booleanValue())))
						{
							...
							hCaso.put("dt_conclusione", dbr.get("ski_data_uscita"));
							String motChiu = (String)h.get("ski_dimissioni");
							String motChiuFlux = getTabVociCodReg(dbc, "ICHIUS", motChiu);
							hCaso.put("motivo", motChiuFlux);

							hCaso.put("operZonaConf", (String)dbr.get("ski_operatore")); // 15/10/09
							LOG.info(" insert() -- Chiudi caso = HashCaso: " + hCaso.toString());
							Integer r = gestore_casi.chiudiCaso(dbc, hCaso);
							LOG.info("insert() -- Ritorno di ChiudiCaso == " + r);
						}
					}
				}
			}
		} else 
			LOG.info("--- SkInfEJB.insert: Contatto NON DOMICILIARE quindi NON si comunicano SEGNALAZ e PRESACAR ---");	
	}
	...
	throws SQLException 
	{
		String methodName = "AggiornaDimissione";
		try
		{
			...
			if(dbrDim!=null)
			{				
				dbrDim.put("data_carico", ""+dbr.get("ski_infermiere_da"));
				
				dbrDim.put("oper_carico", ""+dbr.get("ski_infermiere"));
	...
	ISASRecord dbr_ret = null;
		...
		//gb 30/04/07 **************
			...
			ce.setISASRecord(null);
			try{
				LOG.error("SkInfEJB.update() => ROLLBACK");
				dbc.rollbackTransaction();
			}catch(Exception e1){
		...
		throws Exception
	{
                ...
                debugMessage("SkInfEJB: deleteContattoInProgCont - myselect=[" + myselect + "]");
...
gb 02/05/07: fine *******/
	...
	//gb 02/05/07 *******
	private void deletePianoAssist(ISASConnection dbc, String cartella, String contatto)
	throws Exception
	{
		String myselect="SELECT * FROM piano_assist" +
		" WHERE pa_tipo_oper = '02'" +
		" AND n_cartella = " + cartella +
	...
	//gb 02/05/07 *******
	private void deletePianoAccessi(ISASConnection dbc, String cartella, String contatto)
	throws Exception
	{
	...
	//gb 02/05/07 *******
	private void deletePianoVerifiche(ISASConnection dbc, String cartella, String contatto)
	throws Exception
	{
	...
	SQLException {
		...
		try{
			...
			String data=dbr.get("ski_data_apertura").toString();
			...
			if (ris.equals("N"))
			{
				...
				String[] Vschede={"skiprestaz","skiaccoglienza","skifamiglia","skiterapia","skinorton",
						...
						/*"skiprogass",*/"skiverifica","skinf_referente","skintamb","skiausili_t",
				...
				deletePianoAssist(dbc, cartella, contatto); //gb 02/05/07
				deletePianoAccessi(dbc, cartella, contatto); //gb 02/05/07
				deletePianoVerifiche(dbc, cartella, contatto); //gb 02/05/07
			}
	...
	throws  SQLException {
		...
		try {
			...
			LOG.info("Dentro VerificaInterv=>"+mysel);
	...
	throws DBRecordChangedException, ISASPermissionDeniedException, SQLException {
		...
		try{
			...
			LOG.info("DeleteSchede "+myselect);
			ISASCursor dbcur=dbc.startCursor(myselect);
			while (dbcur.next()){
				ISASRecord dbr = dbcur.getRecord();
	...
	throws DBRecordChangedException,ISASPermissionDeniedException, SQLException {
		...
		try{
			...
			LOG.info("ContattiEJB.Contsan_n(): "+mysel);
	...
	private ISASRecord insertInfRef(ISASConnection dbc, String infref,
		...
		boolean done=false;
		try{
			ISASRecord dbref=dbc.newRecord("skinf_referente");
			
			dbref.put("n_cartella",n_cartella);
			dbref.put("n_contatto",n_contatto);
			
			dbref.put("skir_infermiere",infref);
			dbref.put("skir_infermiere_da",data_ref);
			dbc.writeRecord(dbref);
			done=true;
			return dbref;
		}catch(DBRecordChangedException e){
			LOG.error("SkInfEJB.insertIntRef(): "+e);
			throw e;
		}catch(ISASPermissionDeniedException e){
			LOG.error("SkInfEJB.insertIntRef(): "+e);
			throw e;
		}catch(Exception e1){
			LOG.error("SkInfEJB.insertIntRef(): "+e1);
			throw new SQLException("Errore eseguendo una update() - "+  e1);
		}
	}
	...
	throws SQLException {
		...
		try{
			...
			e.printStackTrace();
			throw new SQLException("SkInf=>CaricaData: Errore: manca la chiave primaria");
		}
	}
	...
	throws SQLException {
		...
		String ski_data_uscita=null;
		//String apertura=null;
		ISASConnection dbc=null;
		try {
			cartella=(String)h.get("n_cartella");
			contatto=(String)h.get("n_contatto");
			//apertura=(String)h.get("ski_data_apertura");
			ski_data_uscita = (String)h.get("ski_data_uscita");
		}catch (Exception e){
			e.printStackTrace();
			throw new SQLException("SkInf query_aggiorna: Errore: manca la chiave primaria");
		}
		try{
			dbc=super.logIn(mylogin);
			...
			" n_cartella="+cartella+
			" and n_contatto="+contatto;
			//" and ski_data_apertura="+formatDate(dbc,apertura);
			ISASRecord dbr=dbc.readRecord(myselect);
			dbr.put("ski_data_uscita", ski_data_uscita);
			dbr.put("ski_data_prest_ric",CaricaData(dbc,cartella,contatto,"skiprestaz","skp_data"));
			dbr.put("ski_data_accogl",CaricaData(dbc,cartella,contatto,"skiaccoglienza","ska_data"));
			dbr.put("ski_data_fam",CaricaData(dbc,cartella,contatto,"skifamiglia","skf_data"));

			// 15/05/06 dalla vers. 06.03
			dbr.put("ski_data_nucleofam",CaricaData(dbc,cartella,contatto,"skinucleo_fam","data_variazione"));
			/*** 16/11/06 m.
        dbr.put("ski_data_caregiver",CaricaData(dbc,cartella,contatto,"skicaregiver","skcg_data"));
        dbr.put("ski_data_braden",CaricaData(dbc,cartella,contatto,"skibraden","skb_data"));
			 ***/
			// 16/11/06 m.
			dbr.put("ski_data_caregiver",CaricaData(dbc, cartella, dbr.get("ski_data_apertura"), "sc_caregiver", dbr.get("ski_data_uscita"), "skcg_data"));
			dbr.put("ski_data_braden",CaricaData(dbc, cartella, dbr.get("ski_data_apertura"), "sc_braden", dbr.get("ski_data_uscita"), "skb_data"));
			dbr.put("ski_data_tiq",CaricaData(dbc, cartella, dbr.get("ski_data_apertura"), "sc_tiq", dbr.get("ski_data_uscita")));
			// fine 15/05/06

			dbr.put("ski_data_terapia",CaricaData(dbc,cartella,contatto,"skiterapia","skt_data"));
			dbr.put("ski_data_norton",CaricaData(dbc,cartella,contatto,"skinorton","skno_data"));
			dbr.put("ski_data_dolore",CaricaData(dbc,cartella,contatto,"skidolore","skid_data"));
			dbr.put("ski_data_ausili",CaricaData(dbc,cartella,contatto,"skiausili_t","skat_data"));
			dbr.put("ski_data_relaz",CaricaData(dbc,cartella,contatto,"skirelaz","skr_dat"));
			dbr.put("ski_data_coscienza",CaricaData(dbc,cartella,contatto,"skicoscienza","skc_data"));
			dbr.put("ski_data_umore",CaricaData(dbc,cartella,contatto,"skiumore","sku_data"));
			dbr.put("ski_data_respiraz",CaricaData(dbc,cartella,contatto,"respiraz","skr_data"));
			dbr.put("ski_data_nutriz",CaricaData(dbc,cartella,contatto,"skinutriz","skn_data"));
			dbr.put("ski_data_escret",CaricaData(dbc,cartella,contatto,"skiescret","ske_data"));
			dbr.put("ski_data_motr",CaricaData(dbc,cartella,contatto,"skimotric","skd_data"));
			dbr.put("ski_data_altro",CaricaData(dbc,cartella,contatto,"skialtro","skx_data"));
			dbr.put("ski_data_decubito",CaricaData(dbc,cartella,contatto,"skidecubito_t","skdt_data"));
			dbr.put("ski_data_ulcere",CaricaData(dbc,cartella,contatto,"skiulcere_t","skut_data"));
			dbr.put("ski_data_lesioni",CaricaData(dbc,cartella,contatto,"skivasco_t","skvt_data"));
			dbr.put("ski_data_alteraz",CaricaData(dbc,cartella,contatto,"skialteraz","skal_data"));
			dbr.put("ski_data_indagini",CaricaData(dbc,cartella,contatto,"skiesami","skes_data"));
			//bargi 10/11/2006 dbr.put("ski_data_adl",CaricaData(dbc,cartella,contatto,"tabadl","data"));
			//bargi 10/11/2006 dbr.put("ski_data_iadl",CaricaData(dbc,cartella,contatto,"tabiadl","data"));
			//bargi 10/11/2006 dbr.put("ski_data_pfeiffer",CaricaData(dbc,cartella,contatto,"pfeiffer","data"));

//			dbr.put("ski_data_adl",CaricaData(dbc, cartella, dbr.get("ski_data_apertura"), "sc_adl", dbr.get("ski_data_uscita")));
//			dbr.put("ski_data_pfeiffer",CaricaData(dbc, cartella, dbr.get("ski_data_apertura"), "sc_pfeiffer", dbr.get("ski_data_uscita")));
//			dbr.put("ski_data_iadl",CaricaData(dbc, cartella, dbr.get("ski_data_apertura"), "sc_iadl",dbr.get("ski_data_uscita")));
//			dbr.put("ski_data_barthel",CaricaData(dbc, cartella, dbr.get("ski_data_apertura"), "sc_barthel", dbr.get("ski_data_uscita"),"skbt_data"));

			LOG.info("SkInfEJB -- query_aggiorna: "+dbr.getHashtable().toString());
			dbc.close();
			super.close(dbc);
			done=true;
			return dbr;
			...
			e.printStackTrace();
			throw new SQLException("Errore eseguendo una query_aggiorna()  ");
		}finally{
			if(!done){
				try{
					dbc.close();
...
//			dbr.put("ski_data_barthel",CaricaData(dbc, cartella, dbr.get("ski_data_apertura"), "sc_barthel", dbr.get("ski_data_uscita"),"skbt_data"));
			...
			e.printStackTrace();
			throw new SQLException("Errore eseguendo una query_aggRecord()  ");
		}finally{
			if(!done){
				try{
					dbc.close();
	...
	throws DBRecordChangedException, ISASPermissionDeniedException, SQLException {
		...
		try{
			...
			" n_cartella="+n_cartella+
			" and n_contatto="+n_contatto;
			LOG.info("select prima di aggiornamento su skinf==="+myselect);
			ISASRecord dbr=dbc.readRecord(myselect);
			
			if (dbr!=null){
				LOG.info("CI ENTROOOOOO");
				dbr.put("ski_infermiere",(String)h.get("ski_infermiere"));
				dbr.put("ski_infermiere_da",java.sql.Date.valueOf((String)h.get("ski_infermiere_da")));
			}
			
			dbc.writeRecord(dbr);
			myselect="Select * from skinf where "+
			" n_cartella="+n_cartella+
			" and n_contatto="+n_contatto;
			dbr=dbc.readRecord(myselect);
	...
	public ISASRecord update_eventi(myLogin mylogin,Hashtable h) throws DBRecordChangedException, ISASPermissionDeniedException, SQLException {
		...
		try{
			...
			" n_cartella="+n_cartella+
			" and n_contatto="+n_contatto;
			LOG.info("update_eventi su skinf==="+myselect);
			ISASRecord dbr=dbc.readRecord(myselect);
			if (dbr!=null){
	...
	throws  SQLException {
		String punto = ver + "query_controlloData ";
		String ritorno="";
		String ritorno_max="";
		ISASConnection dbc=null;
		...
		try {
			...
			" AND int_tipo_oper='" +ISASUtil.getValoreStringa(h, ManagerProfile.TIPO_OPERATORE) +"' ";
			LOG.trace(punto + " query interventi>" + mysel);
			...
			LOG.debug(punto + "Dati che invio>>" +(dbtxt!=null ? dbtxt.getHashtable()+"":"No dati "));
		...
		try{
			...
			for(int i=0;i<vkey.size();i++)
			{
				...
				if (key.equals("ICHIUS")){
					...
					LOG.info("query_comboTAB: "+myselect);
					ISASCursor dbcur=dbc.startCursor(myselect);
					Vector vdbr=dbcur.getAllRecord();
					if(res==null)
						res=new Hashtable();
					...
					LOG.info("query_comboSegnala: "+myselect);
					ISASCursor dbcur=dbc.startCursor(myselect);
					Vector vdbr=dbcur.getAllRecord();
					if(res==null)
						res=new Hashtable();
					...
					LOG.info("query_comboTipute: "+myselect);
					ISASCursor dbcur=dbc.startCursor(myselect);
					Vector vdbr=dbcur.getAllRecord();
					if(res==null)
						res=new Hashtable();
					...
					LOG.info("query_comboMotivoS: "+myselect);
					ISASCursor dbcur=dbc.startCursor(myselect);
					Vector vdbr=dbcur.getAllRecord();
			...
			e.printStackTrace();
			throw new SQLException("Errore eseguendo una query_Allcombo()  ");
		}finally{
			if(!done){
				try{
					dbc.close();
...
tra la data apertura e, se inserita, la data chiusura */
	...
	private void leggiDiagnosi_(ISASConnection mydbc, ISASRecord mydbr) throws Exception
	{
		...
		Object dtApertura = (Object)mydbr.get("ski_data_apertura");
		//gb 27/04/07		Object dtChiusura = (Object)mydbr.get("ski_data_chiusura");
		Object dtChiusura = (Object)mydbr.get("ski_data_uscita"); //gb 27/04/07
	...
	throws  SQLException{
		try {
			...
			LOG.error(ex);
			throw new SQLException("Errore eseguendo una AggiornaData()  ", ex);
		}
	}
	...
	throws DBRecordChangedException, ISASPermissionDeniedException, SQLException
	{
		...
		try{
			...
			throw new SQLException("Errore eseguendo rimuovoAgendaCaricata()  ", e);
		}finally{
			if(!done)
			{
				try{
	...
	public Boolean query_checkContSuccessivi(myLogin mylogin, Hashtable h0)
	{
		...
		try {
			...
			String critSoloAperti = " AND ski_data_uscita IS NULL";
			// 14/04/08 ----

			String myselect = "SELECT * FROM skinf" +
			" WHERE n_cartella = " + cart +
			" AND ski_data_apertura > " + formatDate(dbc, dtRiferimento) +
			(soloAperti?critSoloAperti:""); // 14/04/08

			LOG.info("SkInfEJB: query_checkContSuccessivi - myselect=[" + myselect + "]");
			...
			LOG.error("SkInfEJB.query_checkContSuccessivi - Eccezione=[" + e1 + "]");
			return (Boolean)null;
		} finally{
			if(!done){
				try{
	...
	private void chiudiSkValutaz(ISASConnection mydbc, String numCart, String dtSkVal, String data_chiusura) throws Exception
	{
		...
		LOG.info("SkInfEJB -->> chiudiSkValutaz: mysel=["+mysel+"]");
	...
	public Vector query_contattiAperti(myLogin mylogin, Hashtable h) throws  SQLException, ISASPermissionDeniedException
	{
		...
		try{ 
			...
			" progetto_cont pc" +
			/** 14/04/08
                      " WHERE sk.ski_data_apertura <= " + formatDate(dbc, strDataA) +
                      " AND (sk.ski_data_uscita IS NULL OR sk.ski_data_uscita >= " + formatDate(dbc, strDataDa) + ")" +
			 **/
			// 14/04/08: selezione solo dei contatti aperti nel periodo (x ridurre il num dei rec estratti) ----
			" WHERE sk.ski_data_apertura <= " + formatDate(dbc, strDataA) +
			" AND sk.ski_data_apertura >= " + formatDate(dbc, strDataDa) +
			// 14/04/08 -----------
			" AND pc.prc_tipo_op = '02'" +
			...
			" ORDER BY sk.ski_data_apertura";

			LOG.info("SkInfEJB/query_contattiAperti: "+myselect);
			...
			e.printStackTrace();
			throw new SQLException("Errore eseguendo la query_progettiAperti()  ", e);
		}
		finally  {
			if(!done){
				try{
	...
	{
		LOG.info("SkInfEJB getZonaOper oper == " + oper);
		String zonaOper = "";
	...
	private String getValxZona(String val, String zonaOper)
	{
		...
		if (rit.trim().equals(""))// non esiste codifica x la zona dell'oper (oppure oper senza zona!)
			LOG.info("!!!! SkInfEJB.getValxZona: NON esiste codifica su CONF per la zona=["+zonaOper+"] !!!!");
		return rit;
	}
	...
	private String getDtChiuCasoPrec(ISASConnection dbc, Hashtable h) throws Exception 
	{
		...
		LOG.info("getDtChiuCasoPrec - dtChiusPrec=["+dtChiusPrec+"]");
	...
	{	
		...
		try {
			...
			String sel = "SELECT s.* FROM skinf s"
				+ " WHERE s.n_cartella = " + cart
				+ " AND s.ski_data_apertura IN (SELECT MAX(a.ski_data_apertura)"
						+ " FROM skinf a"
						+ " WHERE a.n_cartella = s.n_cartella"
						+ " AND EXISTS (SELECT c.* FROM caso c"
								...
								+ " AND c.dt_presa_carico <= a.ski_data_apertura"
								+ " AND ((c.dt_conclusione IS NULL)"
										+ " OR (c.dt_conclusione >= a.ski_data_apertura))"
						+ ")"
				+ ")";
			LOG.info("SkInfEJB.leggiLastSkInfInCaso(): sel=["+sel+"]");
			
			ISASRecord dbr2 = dbc.readRecord(sel);
			if (dbr2 != null) {
				h_ret = (Hashtable)dbr2.getHashtable();
				
				java.sql.Date dtPresaCarSql = (java.sql.Date)dbr2.get("ski_dtpresacarico");
				if (dtPresaCarSql != null) {
					DataWI myDtPresaCar = new DataWI(dtPresaCarSql);	
					h_ret.put("ski_dtpresacarico", (myDtPresaCar!=null?myDtPresaCar.getString(0):""));				
				}
			}
			...
			LOG.error("SkInfEJB.leggiLastSkInfInCaso - Eccezione=[" + e1 + "]", e1);
			/**
			LOG.error("***************************");
			e1.printStackTrace();
			LOG.error("***************************");
	...
	public Boolean checkAbilitazioneFlussi(myLogin mylogin, Hashtable h){
		...
		try {
			...
			if((h.containsKey("ski_data_uscita")) 
					&& (!h.get("ski_data_uscita").toString().equals("")))
				condizioneData = " AND pr_data_puac <= " + formatDate(dbc, h.get("ski_data_uscita").toString());
			...
			LOG.info("SkInfEJB: checkAbilitazioneFlussi - myselect= " + myselect);
	...
	public Hashtable getDatiRLPuaUvm(myLogin mylogin, Hashtable h){
		...
		try {
			...
			LOG.info("SkInfEJB: getDatiRLPuaUvm " + h.toString());
			...
			LOG.info("SkInfEJB: getDatiRLPuaUvm - myselect= " + myselect);
	...
	public String getModalita (myLogin mylogin, Hashtable h)
	{
		...
		try 
		{
			...
			{
			LOG.info("Eccezione in Skinf.getModalita(): Manca la chiave primaria");
			dbc.close();
			super.close(dbc);
			return null;
			}
			String myselect = "select * from  skinf where n_cartella = "+n_cartella+" and n_contatto="+n_contatto;
			dbr = dbc.readRecord(myselect);
			if (dbr!=null) ret = dbr.get("ski_modalita").toString();
			else ret=null;
			dbc.close();
			super.close(dbc);
			done=true;
	...
	public Boolean checkEsistenzaRispostaAssistenziale(myLogin mylogin, Hashtable h){
		...
		try {
			...
			LOG.info("SkInfEJB: checkEsistenzaRispostaAssistenziale " + h.toString());
			...
			LOG.info("SkInfEJB: checkEsistenzaRispostaAssistenziale - myselect= " + myselect);
			...
			while(dbcur.next()){
				...
				if(dbr != null){
					String sel = "SELECT * FROM rv_puauvm_rispro" +
								...
								")";
					LOG.info("SkInfEJB: checkEsistenzaRispostaAssistenziale - myselect=[" + sel + "]");
	...
	// 05/02/13
	public String decodPresidio(ISASConnection dbc, String codPres, String codOper) throws Exception
	{
		String ret = "";
	...
	// 05/04/13
	public ISASRecord getSkInf(ISASConnection dbc, String cartella, String dataRif) throws Exception
	{
		ISASRecord rec = null;

		try {
			String sel = " SELECT * FROM skinf WHERE n_cartella = " + cartella
			+ " AND ski_data_apertura <= " + dbc.formatDbDate(dataRif)
			+ " AND ((ski_data_uscita IS NULL) OR (ski_data_uscita >= " + dbc.formatDbDate(dataRif) + "))";

			rec = dbc.readRecord(sel);

			return rec;
		} catch (Exception e) {
			LOG.error("SkInfEJB.getSkInf: ERRORE - e=" + e);
			throw e;
		}
	}	
...
public ISASRecord update(ISASConnection dbc, ISASRecord dbr, ISASCursor dbcur1) throws Exception{
	...
	try 
	{
		strDtSkVal = ISASUtil.getValoreStringa(dbr, "pr_data");//gb 03/05/07

		n_cartella=ISASUtil.getValoreStringa(dbr, "n_cartella");
		n_contatto=ISASUtil.getValoreStringa(dbr, "n_contatto");
		data_apertura=ISASUtil.getValoreStringa(dbr,"ski_data_apertura");
		dimissOsp=ISASUtil.getValoreStringa(dbr, "dimiss");
	}
	catch (Exception e)
	{
		e.printStackTrace();
	...
	String abilMultiCont = (String)leggiConf(dbc, "ABIL_NEWCONT_INF");
	// 15/07/08 -----
	String zonaOper = getZonaOper(dbc, (String)dbr.get("ski_operatore"));
	abilMultiCont = getValxZona(abilMultiCont, zonaOper);
	// 15/07/08 -----
	multiCont = ((abilMultiCont != null) && (abilMultiCont.trim().equals("SI")));
	if (!multiCont) 
	...
	if ((gestore_casi.isUbicazRegVeneto(dbc, h)).booleanValue()) {	
		if ((h.get("ski_descr_contatto") == null) || ("".equals((String)h.get("ski_descr_contatto"))))
			dbr.put("ski_descr_contatto", "CONTATTO INFERMIERISTICO N."+dbr.get("n_contatto"));
	}			
	
	
	LOG.info("SkInf.UPDATE: "+(dbr.getHashtable()).toString());
	dbc.writeRecord(dbr);
	...
	if (dbr != null) {
		...
		if (data_chiusura != null && !data_chiusura.equals(""))
		{
			...
			String strMsgCheckDtCh = clCcec.checkDtChDaContProgGTDtApeDtCh(dbc, n_cartella, n_contatto, data_chiusura, "02");
			...
			clCcec.chiudoDaContattoInGiu(dbc, n_cartella, n_contatto, data_chiusura, "02", (String)dbr.get("ski_operatore"));
			//gb 01/10/07: fine *******
		...
		String infref=(String)dbr.get("ski_infermiere");
		String data_ref=((java.sql.Date)dbr.get("ski_infermiere_da")).toString();
		if (!dbcur.next())
		{
			if(dbr.get("ski_infermiere")!=null && !((String)dbr.get("ski_infermiere")).equals(""))
			{
				infref=(String)dbr.get("ski_infermiere");
				this.insertInfRef(dbc,infref,data_ref,data_apertura,n_cartella,n_contatto);
			}
		}
		
		// Aggiorno l'infermiere referente nel caso in cui sia diverso da quello precedente, altrimenti ne inserisco uno nuovo.
		selref="SELECT * FROM skinf_referente WHERE "+
				"n_cartella="+n_cartella+" AND "+
				"n_contatto="+n_contatto+" AND " +
				"skir_infermiere_da="+formatDate(dbc, dbr.get("ski_infermiere_da").toString());
		ISASRecord dbr_ref = dbc.readRecord(selref);
		if (dbr_ref == null){
			this.insertInfRef(dbc,infref,data_ref,data_apertura,n_cartella,n_contatto);
		}
		else if (!dbr_ref.get("skir_infermiere").toString().equals(dbr.get("ski_infermiere").toString()))
		{
			dbr_ref.put("skir_infermiere",dbr.get("ski_infermiere").toString());
			dbc.writeRecord(dbr_ref);
		}
		
		
		if(dimissOsp!=null && !dimissOsp.isEmpty() && !dimissOsp.equals("N"))
			AggiornaDimissione(dbc, n_cartella, dimissOsp, dbr);
		...
		if ((dbr.get("ski_modalita") != null) && ("1".equals((String)dbr.get("ski_modalita")))) {			
			...
			if(gestore_segnalazioni.isSegnalDaGestire(dbc, GestCasi.CASO_SAN))
			{
				...
				LOG.info("Fatta gestione segnalazione per il caso: " + idCaso);						
...
*** 26/06/13 m: NON SERVE PIU ***/												
			...
			/* RIMOSSO IN QUANDO NON HA SENSO IL CONTROLLO */
//			if (data_chiusura != null && !data_chiusura.equals(""))
//			{
//				LOG.info("Controllo contatto UNICO SANITARIO H == " + h.toString());
//				boolean unico = gestore_casi.query_checkUnicoContAperto(dbc, h, true, true);
//				if (idCaso != -1 && unico)
//				{
//					LOG.info("Gestisco la chiusura del caso" );
//					// E' uguale ad S quando c'e' la possibilita' che ci siano piu' contatti e questo e'
//					// l'ultimo contatto aperto che stiamo chiudendo! Quindi devo chiudere, se esiste, il caso
//					// sociale associato!
//					int origine = -1;
...
//					if (origine != -1)
//					{
//						LOG.info("update() -- Origine del caso: " + origine + " - stato = " + statoCaso);
//						if ((origine == GestCasi.CASO_SAN)
//						&& ((statoCaso == GestCasi.STATO_ATTIVO) 
//							|| (((Boolean)gestore_casi.isUbicazRegTosc(dbc, h)).booleanValue())))
//						{
...
//							// 26/03/10 ----										
//							hCaso.put("operZonaConf", (String)dbr.get("ski_operatore")); // 15/10/09
//							LOG.info(" -- update(): Chiudi caso = HashCaso: " + hCaso.toString());
//							Integer r = gestore_casi.chiudiCaso(dbc, hCaso);
//							LOG.info("Ritorno di ChiudiCaso == " + r);
//						}
//					}
//				}
//			}					
		} else 
			LOG.info("--- SkInfEJB.update: Contatto NON DOMICILIARE quindi NON si comunicano SEGNALAZ e PRESACAR ---");
	...
	public int recuperaMaxContatto(myLogin myLogin, Hashtable<String, String> dati) {
		...
		int nContatto = -1;
		ISASConnection dbc = null;
		ISASRecord dbrSkInf = null;
		boolean done = false;
		try {
			dbc = super.logIn(myLogin);
			String nCartella = ISASUtil.getValoreStringa(dati, CostantiSinssntW.N_CARTELLA);
			String query = "select max(n_contatto) as contatto from skinf where n_cartella = " + nCartella;
			dbrSkInf = dbc.readRecord(query);

			nContatto = ISASUtil.getValoreIntero(dbrSkInf, "contatto");
	...
	public ISASRecord recuperaMaxSkinf(ISASConnection dbc, String nCartella) {
		String punto = ver + "recuperaMaxSkinf ";
		ISASRecord dbrSkInf = null;
		try {
			String query = "SELECT k.* FROM skinf k WHERE k.n_cartella = " +nCartella +
					 " AND k.n_contatto IN (SELECT MAX (x.n_contatto) FROM skinf x " +
	...
	public static Boolean checkRiapriPC(ISASConnection dbc, Hashtable h){
		boolean done = false;
		...
		try {
			...
			String n_cartella = h.get("n_cartella").toString();
			String data_ingresso = h.get("data_apertura").toString();
			
		
			String myselect = "SELECT * from caso where  "+ dbc.formatDbDate(data_ingresso)+
								" between dt_presa_carico and dt_conclusione " +
								" and n_cartella = "+n_cartella+
								" and stato = 2"+
								" and dt_conclusione is not null"+
								" and motivo is not null"+
								" and origine = 1"+
								" and not exists (select * from caso c1 where c1.n_cartella = n_cartella" +
								" and c1.stato = 1 " +
								" and c1.id_caso <> id_caso)";
			
			System.out.println("SkInfEJB: checkRiapriPC - myselect= " + myselect);
			
			ISASRecord dbr = dbc.readRecord(myselect);
			if(dbr != null) {
				dbr.put("dt_conclusione", "");
				dbr.put("motivo", "");
				dbr.put("stato", "1");
				dbr.put("evento_id_in", "-9");
				dbc.writeRecord(dbr);
			}
			done = true;
