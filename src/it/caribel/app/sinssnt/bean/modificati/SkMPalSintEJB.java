...
public class SkMPalSintEJB extends SINSSNTConnectionEJB  
{
	private static String nomeEJB = "SkMPalSintEJB ";
	
	public SkMPalSintEJB() {}
	...
	public ISASRecord queryKey(myLogin mylogin,Hashtable h) throws  SQLException {
		...
		try{
			...
			String myselect="Select * from skmpal_sintomi where "+
			" n_cartella="+(String)h.get("n_cartella")+
			" AND sks_progr="+(String)h.get("sks_progr");
			System.out.println("QueryKey SkMPalSintEJB:"+myselect);
	...
	public ISASRecord queryKeyLast(myLogin mylogin,Hashtable h) throws  SQLException 
	{
		...
		try{
			...
			String mysel = "SELECT * FROM skmpal_sintomi" +
	...
	public Vector query(myLogin mylogin,Hashtable h) throws  SQLException {
		...
		try{
			...
			String myselect="SELECT * FROM skmpal_sintomi WHERE "+
			"n_cartella="+(String)h.get("n_cartella");
			myselect=myselect+" ORDER BY sks_data DESC ";

			System.out.println("query skmpal_sintomi: "+myselect);
	...
	throws DBRecordChangedException, ISASPermissionDeniedException, SQLException 
	{
		...
		try
		{
			...
			ISASRecord dbr = dbc.newRecord("skmpal_sintomi");
			
			//INSERISCO NELLA TABELLA SINTOMI
			/*Mi vado a calcolare il progressivo*/
			String selProg = "SELECT MAX(sks_progr) progr FROM skmpal_sintomi WHERE "+
			...
			String myselect="SELECT * FROM skmpal_sintomi WHERE n_cartella="+n_cartella+
			" AND sks_progr="+progr;
			dbr=dbc.readRecord(myselect);
			System.out.println("select skmpal_sintomi insert: "+myselect);

			// 25/09/07
			if (dbr != null) dbr.put("griglia", (Vector)leggiTuttiRec(dbc, n_cartella));
	...
	{
		String myselect = "SELECT * FROM skmpal_sintomi" +
