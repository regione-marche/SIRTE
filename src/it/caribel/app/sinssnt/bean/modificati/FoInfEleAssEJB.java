...
* CARIBEL S.r.l. - SINSS: produzione elenco assistiti per infermieri
*01/10/2003 -  Bargi
*/
import it.caribel.app.sinssnt.bean.nuovi.FiltroStampaRA;
...
public class FoInfEleAssEJB extends SINSSNTConnectionEJB {

public FoInfEleAssEJB() {}
...
int totaleInf =0;
	...
	private String MIONOME = "11-FoInfEleAssEJB. ";
...
private String getMotivoUscita(ISASRecord dbr) {
	try {
        	//switch (new Integer(((String)dbr.get("ski_dimisioni"))).intValue()) {
                switch (((Integer)dbr.get("ski_dimissioni")).intValue()) {
		case 0: return "";
	...
	String s = "SELECT DISTINCT c.n_cartella, "+
		...
		if (par.get("oper")!=null)
         ...
         s=s+"u.codice,u.descrizione,u.des_distretto des_distr"+
                " FROM cartella c,anagra_c a,skinf ski,"+ ((par.get("socsan")!=null && par.get("socsan").equals("01"))?"ubicazioni_n_soc":"ubicazioni_n") +" u";
...
private String aggiungiCondizioneMotivo(Hashtable par) {
	...
	if (ISASUtil.valida(motivo)&& !motivo.equals("0")){
		condizione= " and ski_motivo = '"+motivo+ "' ";
...
private String getSelectParteWhereSint(ISASConnection dbc, Hashtable par) {
	...
	String myselect="";
	myselect= myselect+ " ski_modalita='1'";
        String data_fine="";
        //controllo data fine
		if (par.get("data_fine") != null)
		{
		  data_fine=(String)(par.get("data_fine"));
		  myselect =myselect+" AND ski.ski_data_apertura<="+formatDate(dbc,data_fine);
        }
    ...
    if (livello!=null && !livello.equals("-1"))
        ...
        //if (par.get("oper")!=null && par.get("oper").equals("SI"))
        if (par.get("oper")!=null)
        myselect =myselect+" AND ski.ski_infermiere = op.codice ";
	...
	String cod_inizio="";
        ...
        if (par.get("codice_inizio") != null)
        {
          ...
          if (!cod_inizio.equals(""))
                //myselect=myselect+" AND ski.ski_infermiere>='"+cod_inizio+"'";
        	  myselect=myselect+" AND ski.ski_infermiere='"+cod_inizio+"'";
        }
	...
	myselect = su.addWhere(myselect, su.REL_AND, "u.codice",
       ...
       myselect += " AND a.n_cartella=c.n_cartella"+
         " AND ski.n_cartella=a.n_cartella"+
...
//	String s ="SELECT DISTINCT c.n_cartella, "+
        String s ="SELECT c.n_cartella, "+
                "c.cognome, c.nome, c.data_nasc,ski.ski_tipocura,"+
				"a.dom_citta, a.dom_indiriz, a.indirizzo, a.citta,a.nome_camp, a.telefono1, "+
				"a.cod_med, " + // 07/06/11
				"ski.ski_data_apertura, ski.ski_data_uscita, ski.ski_dimissioni,"+
// 07/06/11: la join su MEDICI provoca lo scarto di numerosi record		"ski.ski_infermiere,mecogn, menome, ";
				// 07/06/11
				"ski.ski_infermiere, ";		
                //if (par.get("oper")!=null && par.get("oper").equals("SI"))
                   s=s+"op.cognome cognome_oper,op.nome nome_oper, op.dipend_conv,";//29-09-2003 aggiunta divisione per operatore
...
// 07/06/11: la join su MEDICI provoca lo scarto di numerosi record       " FROM cartella c, anagra_c a, medici, skinf ski, "+ ((par.get("socsan")!=null && par.get("socsan").equals("01"))?"ubicazioni_n_soc":"ubicazioni_n") +" u";
				// 07/06/11
                " FROM cartella c, anagra_c a, skinf ski, "+ ((par.get("socsan")!=null && par.get("socsan").equals("01"))?"ubicazioni_n_soc":"ubicazioni_n") +" u";				
                //if (par.get("oper")!=null && par.get("oper").equals("SI"))
                  s=s+", operatori op ";
	...
	s = s +  " descrizione_zona,des_distr,descrizione,"+
			"c.cognome,c.nome,c.n_cartella";
	debugMessage("FoInfEleAssEJB.getSelectAnalitica(): "+s);
	return s;
}
...
//			ht.put("#assistenza#", "ASSISTENZA DOMICILIARE");
			assistenza = "ASSISTENZA DOMICILIARE";
		}else if (ass.equals("2")) {
//			ht.put("#assistenza#", "ASSISTENZA AMBULATORIALE");
			assistenza = "ASSISTENZA AMBULATORIALE";
		}else  {
//			ht.put("#assistenza#", "");
			assistenza = "";
		}*/
...
private void mkTotaleMed(mergeDocument doc) {
        Hashtable ht = new Hashtable();
	ht.put("#descrizioneTotMed#", " Totale n. assistiti per Infermiere: ");
...
*/
public byte[] query_infsint(String utente, String passwd, Hashtable par,
	...
	try {
		...
		if (dbcur == null) {
			doc.write("messaggio");
			doc.write("finale");
			debugMessage("FoInfEleAssEJB.query_infsint(): "+
				"cursore nullo.");
		} else  {
...
**/
                ...
                //***
                    ht3.put("#ski_data_apertura#",getDateField(dbr, "ski_data_apertura"));
                    ht3.put("#ski_data_uscita#",getDateField(dbr, "ski_data_uscita"));
...
private void mkAnaliticaContatti(ISASConnection dbc, String n_cartella,
	...
	try {
		...
		if (dbcur == null) {
			ht.put("#ski_data_apertura#", " ");
        		ht.put("#ski_data_uscita#"," ");
		...
		ht.put("#ski_data_apertura#", "*** ERRORE ***");
		ht.put("#ski_data_uscita#"," ");
		ht.put("#ski_dimissioni#", " ");
		doc.writeSostituisci("contatti", ht);
	}
...
*/
public byte[] query_inf(String utente, String passwd, Hashtable par,
	mergeDocument doc) throws SQLException {
	...
	String type = par.get("TYPE")!=null?par.get("TYPE").toString():"PDF";
		...
		if (dbcur == null) {
			doc.write("messaggio");
			doc.write("finale");
			debugMessage("FoInfEleAssEJB.query_inf(): "+
				"cursore nullo.");
		} else  {
			if (dbcur.getDimension() <= 0) {
				doc.write("messaggio");
...
private String getSelectContatti(ISASConnection dbc, Hashtable par,
	...
	selConta = "SELECT ski_data_apertura data_inizio, "+
		"ski_data_uscita data_fine,ski_dimissioni FROM skinf s "+
