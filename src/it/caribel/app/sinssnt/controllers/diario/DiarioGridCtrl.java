package it.caribel.app.sinssnt.controllers.diario;
...
public class DiarioGridCtrl extends CaribelGridCRUDCtrl implements AskDataInput {

	private static final long serialVersionUID = 1L;
	private String myKeyPermission = ChiaviISASSinssntWeb.RMDIARIO;
	private RMDiarioEJB myEJB = new RMDiarioEJB();
	public static final String myPathFormZul = "/web/ui/sinssnt/diario/diarioGrid.zul";
	public static final String myPathFormZulStorico = "/web/ui/sinssnt/diario/diarioGridStorico.zul";
...
//			cbx_tipo_operatore.setSelectedValue(tipo_operatore);
			...
			doLoadGrid();
			if((arg.get("mode") !=null && arg.get("mode").equals("overlapped"))){
	...
	protected void notEditable() {
		if(super.stato_corr.equals(STATO_UPDATE)){
			data_diario.setReadonly(true);
		}else{
			data_diario.setReadonly(false);
		}
	}
	
	protected void afterSetStatoInsert() {
		data_diario.setToDay();
	}
	...
	protected boolean doValidateForm() {
		try{
			if(this.data_apertura_contatto !=null){
				...
				if(data_diario.getValue().before(dtAperturaCont)){
