...
public class ReportStatRiepPazMeseCtrl extends CaribelFormCtrl {
	...
	public void doStampa() {
		try {
		    ...
		    String report = "paz_acquisiti.fo";
		    String type = "PDF";

		    if (tp.equals("2"))
		    {
		        report = "paz_acquisiti.html";
		        type = "application/vnd.ms-excel";
		    }
		    String servlet="/SINSSNTFoServlet/SINSSNTFoServlet"+
		    		"?EJB=SINS_FOPAZACQUISITI"+
		    		"&USER="+CaribelSessionManager.getInstance().getMyLogin().getUser()+
		    		"&ID509="+CaribelSessionManager.getInstance().getMyLogin().getPassword()+ 
		    		"&METHOD=query_pazacquisiti&data_inizio="+
		            data_inizio+"&data_fine="+data_fine+"&dom_res="+tipo_ubi+"&socsan="+socsan+
		            "&ragg="+ragg+"&pca="+pca+"&distretto="+distr+
		            "&zona="+zone+"&TYPE=" + type + "&REPORT=" + report+"&tipo="+tipo;
