...
public class ReportEleAssOpeCtrl extends CaribelFormCtrl {
		...
		try {
            ...
            if (an.isSelected())
                servlet="/SINSSNTFoServlet/SINSSNTFoServlet"+
			    		...
			    		"&METHOD=query_inferef_2&op_ini="+op_ini+"&op_fine="+op_fine+
			    		"&data_inizio="+data_inizio+"&data_fine="+data_fine+
			    		"&distretto="+distr+"&figprof="+figprof+"&ragg="+ragg+
			    		"&pca="+pca+"&zona="+zone+"&REPORT="+report_an+ "&TYPE=" + TYPE;
            else
                servlet="/SINSSNTFoServlet/SINSSNTFoServlet"+
			    		"?EJB=SINS_FOIASSELE2"+
			    		"&USER="+CaribelSessionManager.getInstance().getMyLogin().getUser()+
			    		"&ID509="+CaribelSessionManager.getInstance().getMyLogin().getPassword()+ 
			    		"&METHOD=query_infesint_2&op_ini="+op_ini+"&op_fine="+op_fine+
			    		"&data_inizio="+data_inizio+"&data_fine="+data_fine+
			    		"&distretto="+distr+"&figprof="+figprof+"&ragg="+ragg+
			    		"&pca="+pca+"&zona="+zone+"&REPORT="+report_si+ "&TYPE=" + TYPE;
            	}
            	else
            	{
            	if (an.isSelected())
