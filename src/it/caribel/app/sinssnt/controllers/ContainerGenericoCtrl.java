...
import it.caribel.app.sinssnt.controllers.interfacce.IContainerGenericoCtrl;
...
public class ContainerGenericoCtrl extends ContainerSinssntCtrl implements IContainerGenericoCtrl {

	private static final long serialVersionUID = 3880792706352844956L;

	public static final String myPathZul = "/web/ui/containerGenerico.zul";
	...
	public ComponentInfo doBeforeCompose(Page page, Component parent,ComponentInfo compInfo){
		try{
			...
			if(iu!=null && !iu.canIUse(ChiaviISASSinssntWeb.CONTATTO_GENERICO+myTipoOpInstance)){
				Messagebox.show(
...
//					hashChiaveValore.put("motivo", motivo);
	...
	public void gestisciTopMenu(Component comp){
		...
		if(comp!=null){
			...
			if(comp.getId().equals("cartellaForm")){
				...
				String skfpg_data_apertura = ((CaribelDatebox)comp.query("#skfpg_data_apertura")).getRawText();
				String testo = "";
				...
				if(n_contatto==null || n_contatto.equals(""))
					...
					testo = Labels.getLabel("caribelContainer.contatto.numero")+": "+n_contatto+
							" "+Labels.getLabel("caribelContainer.contatto.data_apertura")+" "+skfpg_data_apertura;
				
				
				String skfpgDataUscita = ((CaribelDatebox)comp.query("#skfpg_data_uscita")).getRawText();
				if(ManagerDate.validaData(skfpgDataUscita)){
					testo+=" " +Labels.getLabel("SchedaInfForm.conclusione.data.linea.menu")+": "+ skfpgDataUscita;
				}
				
				addItemMenuTop(comp,testo,  (ManagerDate.validaData(skfpgDataUscita) ? CostantiSinssntW.CTS_ROW_RED:""));
			}else if(comp.getId().equals(CostantiSinssntW.CTS_DIARIO)){
				logger.debug(punto + " diario>>");
			}
		}
	...
	public void gestisciLeftMenu(Component comp){
		...
		if(hashChiaveValore.get(CostantiSinssntW.N_CARTELLA)!=null){
			//Abilito Contatto
			disableButtonMenuLeft("contattoGenForm",false);
			//Abilito Storico contatti
			disableButtonMenuLeft("contattoGenGridSto",false); 
	...
	private void gestisciApertureAutomatiche(Component comp){
		...
		if(comp!=null){
			...
			if(comp.getId().equals("cartellaForm")){
				...
				String n_cartella = ((CaribelIntbox)comp.query("#n_cartella")).getRawText();	
				if(super.hashIdComponent.get("contattoGenForm")==null &&  n_cartella!=null && !n_cartella.equals("")){
					recuperaDaSchedaValutazione(n_cartella);
					HashMap<String, Object> mapParam = new HashMap<String, Object>();
					mapParam.put(CostantiSinssntW.N_CARTELLA, n_cartella);					
					Component contattoGenForm = createComponent(hIdUrizul.get("contattoGenForm"), mapParam);
					showComponent(contattoGenForm);
				}
			} else if(comp.getId().equals("contattoGenForm") && aprireSegnalazione){
...
//		        hashChiaveValore.put("stato", new Integer(stato));
	...
	@Listen("onClick=#btn_contattoGenForm")
	public void btn_contattoGenForm() {
		try{
			if(formNeedApprovalForSave()){			
				executeApprovalForSaveBeforShowComponent("contattoGenForm",hIdUrizul.get("contattoGenForm"),true,null);
			}else{
				removeComponentsFrom("contattoGenForm");
				super.showComponent("contattoGenForm",hIdUrizul.get("contattoGenForm"));
			}
		}catch(Exception e){
			doShowException(e);
		}
	...
	@Listen("onClick=#btn_contattoGenGridSto")
	...
	public void removeComponentsFrom(String idComp){
		if(idComp!=null && !idComp.equals("")){
			if(idComp.equals("cartellaForm") || idComp.equals("fassiGrid")){
				...
				super.removeComponent(hashIdComponent.get("contattoGenForm"));
			...
			}else if(idComp.equals("contattoGenForm") || idComp.equals("contattoFisioGridSto")){
				hashChiaveValore.remove(CostantiSinssntW.N_CONTATTO);
				hashChiaveValore.remove(CostantiSinssntW.CTS_ID_SKSO);
				hashChiaveValore.remove("skfpg_data_apertura");
				hashChiaveValore.remove("smri_progr");
